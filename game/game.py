...
class Game:
    ...
    def set_state(self, state: GameState):
        ...
        for row in range(0, len(state.static_board)):
            new_state.static_board.append([])
            for cell in range(0, len(state.static_board[0])):
                new_state.static_board[row].append(state.static_board[row][cell])

        new_state.player = (state.player[0], state.player[1])
